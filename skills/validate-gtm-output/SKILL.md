...
| Respond to an RFP/RFI or build a sales proposal — "help me answer this RFP", "draft a proposal for [account]", win themes, executive summary | Requirements list → issuer + competitive research (parallel) → kickoff pair → draft per requirement → validate | `references/rfp-and-proposals.md` |
...
- **Preserve `score_content` chains**: capture the `content_id` from the first successful score and pass it unchanged when scoring every revision of the same artifact against the same persona — even after an extensive rewrite. Never reuse it for another artifact or persona, and never invent one after a failed call or a response with no ID. Reuse IDs available in the current conversation context; clients must persist and supply them to continue a chain across sessions.
