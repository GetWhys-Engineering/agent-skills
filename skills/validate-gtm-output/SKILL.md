...
| Write or rewrite outward-facing content — blog posts, emails, landing pages, web copy, social posts, ads, white papers, case studies, campaign briefs, sales decks and one-pagers, solution briefs, launch briefs, release notes | `get_brand_voice` + `get_all_messaging_frameworks` (parallel) → select relevant `use_case` frameworks (`list_messaging_frameworks`→`get_messaging_framework`) → persona resolution → optional research grounding → draft → score loop | `references/content-creation.md` |
...
- **Content-gen kickoff is always the pair** `get_brand_voice` + `get_all_messaging_frameworks` (parallel) — `get_all` returns the org's **global (always-on)** frameworks. Then select any relevant **`use_case`** frameworks via `list_messaging_frameworks`→`get_messaging_framework` (this selection IS part of content gen). Never reduce the org's voice to a single framework for general "on-brand" copy — the org-wide voice comes from the `get_all` globals.
...
- **Empty-workspace responses** ("No personas found…", "No brand voice characteristics configured…", "No messaging frameworks configured…") mean the org hasn't configured that artifact — don't retry; proceed without it and point the user to the GetWhys app. **Exception:** `get_all_messaging_frameworks` returning "No global (always-on) messaging frameworks configured…" means only that there are no *global* frameworks — call `list_messaging_frameworks` to check for situational (`use_case`) ones rather than treating it as a dead-end.
